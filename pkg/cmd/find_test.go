...
func TestEveryFlagParses(t *testing.T) {
	...
	command := newCmdFind(options)
	args := []string{
		"--all-namespaces", "--annotate", "owner=platform", "--annotations",
		"owner,team", "--as", "alice", "--as-group", "developers", "--as-uid",
		"1000", "--as-user-extra", "scope=read", "--cache-dir",
		"/tmp/kubectl-find-cache", "--certificate-authority", "/tmp/ca.crt",
		"--client-certificate", "/tmp/client.crt", "--client-key", "/tmp/client.key",
		"--cluster", "cluster-a", "--drain-chunk-size", "500", "--context",
		"context-a", "--cordon", "--delete", "--drain-delete-emptydir-data",
		"--disable-compression", "--drain-disable-eviction", "--drain", "--exec",
		"echo ok", "--force", "--drain-grace-period", "30", "--controller",
		"apps/daemonsets", "--host", "host-1", "--image", "nginx",
		"--insecure-skip-tls-verify", "--jq", ".metadata.name != null",
		"--drain-ignore-daemonsets", "--kubeconfig", "/tmp/config", "--labels",
		"app,version", "--max-age", "24h", "--min-age", "1h", "--name",
		"prod", "--namespace", "production", "--natural-sort", "--node", "host-2",
		"--node-condition", "Ready=True", "--node-labels",
		"topology.kubernetes.io/zone", "--patch", `{"metadata":{}}`,
		"--drain-pod-selector", "app=nginx", "--proxy-url", "http://proxy.example.test",
		"--request-timeout", "5s", "--restarted", "--selector", "app=nginx",
	...
	expected := map[string]bool{
		...
		"cordon":                             true,
		"delete":                             true,
		"drain-delete-emptydir-data":         true,
		"disable-compression":                true,
		"drain-disable-eviction":             true,
		"drain":                              true,
		"controller":                         true,
		"exec":                               true,
		"force":                              true,
		"drain-grace-period":                 true,
		"host":                               true,
		"image":                              true,
		"drain-ignore-daemonsets":            true,
		...
		"skip-confirm":                       true,
		"status":                             true,
		"drain-skip-wait-for-delete-timeout": true,
		"drain-timeout":                      true,
